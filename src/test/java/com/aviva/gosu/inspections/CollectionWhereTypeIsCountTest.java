 ...
 */
public class CollectionWhereTypeIsCountTest extends InspectionsTestBase {
...
//        ((GosuElementVisitor)new CollectionWhereFirst().buildVisitor(problemsHolderMock, false)).visitBeanMethodCallExpression(beanMethod1);
        ...
        for (PsiElement beanMethod : beanMethods) {
            if ( beanMethod instanceof GosuFieldAccessExpressionImpl ) {
                ((GosuElementVisitor)new CollectionWhereTypeIsCount().buildVisitor(problemsHolderMock, false)).visitFieldAccessExpression((GosuFieldAccessExpressionImpl) beanMethod);
            }
        }
